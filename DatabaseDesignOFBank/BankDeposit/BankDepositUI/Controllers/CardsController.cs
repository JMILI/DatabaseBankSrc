...
namespace BankDepositUI.Controllers
{
    public class CardsController : Controller
    {
        ...
        #region “活期存款”功能 已实现
        public IActionResult FloatDeposit()
        {
            return View();
        }
        /// <summary>
        ...
        /// <returns></returns>
        public IActionResult FloatDepositInformation(double money)
        {
        ...
        //用户输入正确，存款成功
        public IActionResult DepositorySuccess()
        {
            return View();
        }
        public IActionResult DepositoryMoneyError()
        {
            return View();
        }
        ...
        public IActionResult FlowBalanceDisplay()
        {
            List<Double> record = new List<Double>();//double类型的List，存放一个余额，一个利息
            record = cardServive.FlowBalanceService((int)DAndC().Dcid);//这里需要service返回一个余额，一个利息
            ViewBag.rate = record[0];
            ViewBag.balance = record[1];
            return View(ViewBag);
        }
        #endregion
